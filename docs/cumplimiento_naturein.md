...
- Principios SOLID reforzados: limpieza de `layout`, headers por rol y `RoleSidebar` abstraído a UI.
...
- Consistencia visual y responsividad del header: botones alineados al estilo del `AuthForm` y menú móvil.
...
- Selección de header por estado de autenticación en `BaseLayout`. `frontend/src/templates/BaseLayout.tsx:24`.
- Organización de `layout`: solo `Footer.tsx`, `HeaderGuest.tsx`, `HeaderUser.tsx`. `frontend/src/components/layout/*`.
- `RoleSidebar` movido a UI para desacoplar layout. `frontend/src/components/ui/RoleSidebar.tsx:1-40`.

## Principios de Diseño (SOLID)
- SRP: separación de vistas y servicios; componentes UI sin lógica de negocio. Ej.: `ExploreCatalogPage` solo orquesta búsqueda y render. `frontend/src/modules/content/ExploreCatalogPage.tsx:27-45`. En `layout` se refuerza SRP con headers por rol y `RoleSidebar` fuera de `layout`.
...
- Capacidad de interacción: UI minimalista, móvil-first, etiquetas claras y modos por rol; header responsivo con menú móvil y botones consistentes. `frontend/src/components/layout/HeaderGuest.tsx:37-46`, `frontend/src/components/layout/HeaderGuest.tsx:49-60`, `frontend/src/components/layout/HeaderUser.tsx:46-59`, `frontend/src/components/layout/HeaderUser.tsx:62-72`.
...
- Mantenibilidad: separación por módulos, documentación en `docs`, pruebas backend presentes. `backend/authservice/tests/test_auth.py`. Limpieza de `layout` reduce duplicidad y mejora organización.
...
- Header por rol seleccionado en `BaseLayout`. `frontend/src/templates/BaseLayout.tsx:24`.
- Botón “Regístrate” en header con estilo del `AuthForm` (verde, bordes rectos, sombra). `frontend/src/components/layout/HeaderGuest.tsx:44`, `frontend/src/components/layout/HeaderGuest.tsx:58`.
- Botón “Ingresar” en header con estilo secundario (blanco, borde verde, bordes rectos, sombra). `frontend/src/components/layout/HeaderGuest.tsx:45`, `frontend/src/components/layout/HeaderGuest.tsx:59`.
- Botones de usuario: “Configuración” y “Salir” con estilos consistentes del secundario del `AuthForm`. `frontend/src/components/layout/HeaderUser.tsx:51`, `frontend/src/components/layout/HeaderUser.tsx:58`, `frontend/src/components/layout/HeaderUser.tsx:71`.
